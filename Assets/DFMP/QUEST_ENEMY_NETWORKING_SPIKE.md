...
DFMP's MVP keeps one native Daggerfall quest state machine per character. Quest NPCs, dialogue, journal state, placed items, rewards, and faction effects are personal client state. This avoids synchronizing arbitrary quest scripts and the large, fragile quest graph replication attempted by the reference co-op fork.
